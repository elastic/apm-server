...
class Gunicorn:
    # todo make this nicer / more resilient
    SLEEP = 3
    ...
    def __enter__(self):
        self.pid = start_gunicorn(self.w)
        time.sleep(self.SLEEP)
        logger.info("Gunicorn ready {}".format(self.pid))
...
def check(index_name, count):
    ...
    for hit in lookup(es.search(index, body=transactions_query), 'hits', 'hits'):
        ...
        assert datetime.utcnow() - timedelta(minutes=2) < timestamp < datetime.utcnow(), \
