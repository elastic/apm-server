...
resource "aws_instance" "apm" {
  ...
  provisioner "file" {
    destination = "/tmp/local-elastic-agent.yml"
    content = templatefile("${path.module}/elastic-agent.yml.tftpl", {
      ...
      apm_version            = "${module.ec_deployment.stack_version}"
  ...
  provisioner "remote-exec" {
    inline = [
      "curl ${data.external.getlatestelasticagent.result.deb} -o elastic-agent.deb && curl ${data.external.getlatestelasticagent.result.rpm} -o elastic-agent.rpm",
...
data "external" "getlatestelasticagent" {
  program = ["bash", "./latest_elastic_agent.sh"]
}
