...
type Generator struct {
	...
	processedTypes map[string]interface{}

	typString, typInt, typInterface string
}
...
func NewGenerator(importPath string, pkg string, typPath string,
	...
	loaded, err := loadPackage(fmt.Sprintf("%s/%s", importPath, pkg))
	if err != nil {
		return nil, err
	}
	...
	g := Generator{
		...
		processedTypes: make(map[string]interface{}),
		typString:      fmt.Sprintf("%s.String", typPath),
		typInt:         fmt.Sprintf("%s.Int", typPath),
		typInterface:   fmt.Sprintf("%s.Interface", typPath),
	}
	for _, r := range root {
...
`[1:], maxVal, ruleMaxVals, maxVal, flattenedName)
				...
				fmt.Fprintf(&g.buf, `
default:
	return fmt.Errorf("validation rule '%s(%s)' violated for '%s' for key " + k)
}
