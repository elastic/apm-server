...
- Fixes Elasticsearch apm-data plugin to fallback to ILM policy for datastreams using default APM integration and not explicitly migrated to Datastream Lifecycle (DSL). More details are available in the [APM known-issues page](https://www.elastic.co/guide/en/observability/current/apm-known-issues.html) {pull}13918[13918]
