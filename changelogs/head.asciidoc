...
- `apm-server.sampling.tail` now requires `apm-server.data_streams.enabled` {pull}5952[5952]
- the `traces-sampled-*` data stream is now `traces-apm.sampled-*` {pull}5952[5952]
