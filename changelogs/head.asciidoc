...
- Improve APM UI query performance by introducing `10m` and `60m` aggregation intervals for transaction metrics. Store them into dedicated data streams `metrics-apm.transaction.${interval}` {pull}9846[9846]
- Improve APM UI query performance by introducing `10m` and `60m` aggregation intervals for span destination metrics. Store them into dedicated data streams `metrics-apm.span_destination.${interval}` {pull}9926[9926]
