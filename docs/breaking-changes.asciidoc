...
[float]
==== 7.6
* APM Server now respects {apm-server-ref-v}/ilm.html#ilm-setup-overwrite-config[`apm-server.ilm.setup.overwrite`]
when running `setup --index-management`.
Previously, when running APM Server, `apm-server.ilm.setup.overwrite` was ignored and
always set to the same value as `apm-server.ilm.setup.enabled`.
...
See {apm-server-ref}/ilm.html#ilm-default[default policy] for more information.
