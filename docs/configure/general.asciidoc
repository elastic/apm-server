...
Wait for the `apm` {fleet} integration to be installed by {kib}. Requires either the <<kibana-enabled,Kibana endpoint>> being enabled
or for the <<elasticsearch-output, {es} output>> to be configured.
Defaults to true.

|====
| APM Server binary | `datastreams.wait_for_integration`
| Fleet-managed     | N/A
|====
...
|====
| APM Server binary | `data_streams.namespace`
