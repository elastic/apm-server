...
* <<transactions,*Transactions*>> describe an event captured by an Elastic {apm-agent} instrumenting a service.
They are the highest level of work being measuring within a service.
* <<transaction-spans,*Spans*>> belong to transactions. They measure from the start to end of an activity,
