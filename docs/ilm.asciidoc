...
You can define a custom `rollover_alias` per event type.

NOTE: Ensure that the configured aliases for all event types start with the same prefix,
...
The example below shows how to change the rollover alias to some custom value,
and setting the template configuration to a matching name and pattern.
