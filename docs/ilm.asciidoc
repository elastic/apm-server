...
You can define a custom index suffix per event type. The index suffix can only contain a
...
This allows to use the rollover alias instead of specific indices in queries.

For example the default rollover alias for event type `transaction` would be `apm-7.8.0-transaction` and points
to a write index `apm-7.8.0-transaction-000001`. When the index reaches the defined size or age it will roll over to
...
For example, the APM Server was started without any customization, leading to a default index setup. Afterwards the
Server configuration is customized for the event type `span` to set up a custom index suffix `production`.
When restarting the Server it will set up the new index template considering the custom part.
A newly created index `apm-server-7.9.0-span-production` would now match the default template with the index pattern
`apm-server-7.9.0-span*` but also the new template with the index pattern `apm-server-7.9.0-span-production*`.
In such a case the old template needs to be manually deleted,
or the `index_pattern` or the `order` need to be changed to avoid conflicts.

NOTE: In case you customize the configuration for `setup.template.pattern`, ensure that the configured pattern
still matches the rollover aliases. Otherwise the Elasticsearch index template with the predefined mappings will
