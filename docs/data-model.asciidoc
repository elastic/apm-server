...
Another example is that `transaction.name` contains just a few unique values and stays within the transaction groups limit,
but `service.name` has high cardinality and reaches the max services limit. Any transactions with new `service.name` will
be aggregated under `service.name`: `_other` and `transaction.name`: `_other`.
