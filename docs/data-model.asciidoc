...
+
--
These metric measures the aggregated latency and latency distribution of transaction groups,
...
+
--
These metric measures the aggregated latency and latency distribution of service transaction groups,
used to power visualizations and analytics in Elastic APM.
...
For transaction, service-transaction, service-destination, and service-summary metrics, there are limits on the number
of unique aggregation groups. When a dimension has a high cardinality and exceeds the limit, the metrics will be aggregated
