...
+
--
These metrics measure the aggregated latency and latency distribution of transaction groups,
...
For transaction metrics, the limits are 1000 services per GB of APM Server, 5000 transaction
groups per GB of APM Server, and 500 transaction groups per service per GB of APM Server.
...
of unique transaction groups tracked, any transactions with new `transaction.name` will all be aggregated under
