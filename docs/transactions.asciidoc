...
A transaction can for example be a HTTP request or an asynchrounous background job.

It contains the timestamp and duration of the event,
a unique id, type, name,
a result (e.g. response code), some contextual data, and other relevant information depending on the agent.
