...
Minifying JavaScript bundles is a common practice;
...
First, set the service version of your application while initializing the RUM Agent.
...
This `serviceVersion` could be the version from your `package.json`. For example:
...
APM Server uses this `serviceVersion` to match the correct source map file to each stack trace.
...
.Click here to expand a sample source map file
[%collapsible%closed]
=====
[source,json]
----
...
Each example includes the four fields necessary for APM Server to identify the correct `source map`.
...
Add the secret token to your curl command using an Authorization header:
...
Or, add an API key to your curl command using an Authorization header:
