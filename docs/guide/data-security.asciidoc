...
There are two ways to filter or obfuscate other types APM data:
...
you can use a series of ingest {ref}/processors.html[processors].
Create a pipeline with a simple description and an empty array of processors:
...
Because the agent captures the request body as a string, you must use the
{ref}/json-processor.html[JSON processor] to convert the field value into a structured JSON object.
We'll save this JSON object in a new field:
...
Next, use the {ref}/remove-processor.html[remove processor] to remove the `body.original` field
if the new `body.original_json` field was created:
...
Finally, {ref}/set-processor.html[set] the value of the new `body.original_json` field to `"redacted"`
if the new `body.original_json` field was created:
...
Use the {ref}/put-pipeline-api.html[create or update pipeline API] to register the new pipeline in Elasticsearch.
We'll call this processor `apm_redacted_body_password`:
...
To make sure the new `apm_redacted_body_password` pipeline works correctly,
test it with the {ref}/simulate-pipeline-api.html[simulate pipeline] API.

The request below simulates three different documents:
...
<2> This document only contains unrelated fields
...
As you can see, only the first simulation was successful as it was the only document with
a valid `password` field in the original HTTP body text. All other documents are unaffected.

The final step in this process is to add the newly created `apm_redacted_body_password` pipeline
to the default `apm` pipeline. This ensures all APM data ingested into {es} will run through the processor.
