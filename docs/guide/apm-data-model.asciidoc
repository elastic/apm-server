...
Spans are stored in {apm-server-ref}/span-indices[span indices].
Note that these indices are separate from {apm-server-ref}/transaction-indices[transaction indices] by default.

[float]
[[dropped-spans]]
==== Dropped Spans

For performance reasons, APM agents can choose to purposefully sample or omit spans.
...
To avoid this, Agents can drop spans. When they do this,
...
are available in the relevant {apm-agents-ref}/index.html[APM Agent] documentation.
