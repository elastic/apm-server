...
* Structured filters -- You now have the ability to filter transactions by type,
result or host/agent name provide you with an easier and a powerful way to explore your APM data.
...
* Log integration -- Navigate from a distributed trace to relevant logs via the APM App UI without losing trace context.
* RUM Angular instrumentation -- Out of the box {apm-rum-ref}/angular-integration.html[Angular instrumentation].
Instrument your SPA written in Angular.js without the need to manually create or rename transactions.
Check out the Angular integration doc and instrument your Angular SPA today!
* JAVA ECS Logging library -- Easily convert your logs to ECS-compatible JSON without creating an additional pipeline.
* .NET agent full framework support -- Out of the box instrumentation for .NET framework.
This means you wont have to use APIs to get your ASP.NET web applications with Elastic APM, simply plug and play!
