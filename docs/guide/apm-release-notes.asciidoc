...
// tag::notable-v72-highlights[]
*APM Server*

* Added support for {apm-server-ref-v}/ilm.html[index lifecycle management (ILM)]:
ILM automates the management of your indices over time,
by rolling over data to new indices when the existing indices reach the specified size or age.
...
Pipelines must be {apm-server-ref-v}/configuration-process.html#register.ingest.pipeline.enabled[enabled].
...
Java is the first agent to get custom metrics in the APM UI, with more agents to follow.

*APM agents*

* The .NET agent is now in beta!
The {apm-dotnet-ref-v}/intro.html[.Net agent] automatically instruments applications using ASP.NET Core 2.x+,
Entity Framework Core 2.x+, and the `HttpClient` class.
Other frameworks can be manually instrumented using the {apm-dotnet-ref-v}/public-api.html[Public API].
// end::notable-v72-highlights[]
