...
To instrument AWS Lambda Node.js functions, follow the official https://aws-otel.github.io/docs/getting-started/lambda/lambda-js[AWS Distro for OpenTelemetry Lambda Support For JS].

Noteworthy configuration elements:

IMPORTANT: Due to a limitation in AWS Lambda Otel Layer for Node.js v0.19 (see https://github.com/aws-observability/aws-otel-lambda/issues/118 ), it is required to create a trace in the calling invocation of the Node.js lambda functions in order to force the sampling strategy to `sampled:true` passing a `traceparent` header with `sampled=true` i.e. with `-01` at the end of the value. 
Creating a trace in the calling invocation of the Lambda function can be achieved using these or other methods that result in `traceparent` header added to the outgoing HTTP call:
* If the caller is a web browser, instrumenting the caller with [Elastic RUM](https://www.elastic.co/guide/en/apm/get-started/current/rum.html) [enabling distributed tracing on the RUM agent](https://www.elastic.co/guide/en/apm/agent/rum-js/5.x/distributed-tracing-guide.html).
* If the caller is an application, instrumenting this application with an OpenTelemetry or Elastic APM agent ensuring distributed tracing is enabled.
...
* Configure the AWS Lambda Node.js function with:
** https://docs.aws.amazon.com/lambda/latest/dg/API_Layer.html[Function
layer]: The latest https://aws-otel.github.io/docs/getting-started/lambda/lambda-js[AWS
Lambda layer for OpenTelemetry] (e.g. `arn:aws:lambda:eu-west-1:901920570463:layer:aws-otel-nodejs-ver-0-19-0:1`)
** https://docs.aws.amazon.com/lambda/latest/dg/API_TracingConfig.html[TracingConfig / Mode] set to `PassTrough`
...
** Export the environment variables:
*** `AWS_LAMBDA_EXEC_WRAPPER="/opt/otel-handler"` for wrapping handlers proxied through the API Gateway (see https://aws-otel.github.io/docs/getting-started/lambda/lambda-js#enable-auto-instrumentation-for-your-lambda-function[here])
...
* Sample Terraform code: https://github.com/michaelhyatt/terraform-aws-nodejs-api-worker-otel/tree/v0.19
