...
For more information on setting up OpenTelemetry, see the OpenTelemetry's {ot-collector}[Getting Started]
documentation.
...
. Select *TSVB*.
. Enter the label name, System load per host IP, and then select the following:
+
* Aggregation: `Average`.
