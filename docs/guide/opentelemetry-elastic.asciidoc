...
* If the caller is a web browser, instrument the caller using (https://www.elastic.co/guide/en/apm/get-started/current/rum.html)[Real User Monitoring (RUM)] by enabling https://www.elastic.co/guide/en/apm/agent/rum-js/5.x/distributed-tracing-guide.html[distributed tracing] on the RUM agent.
