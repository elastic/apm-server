...
It allows you to monitor software services and applications in real time (this guide will indiscriminately use the word service),
...
It receives data from the agents through a JSON API, process it, and stores it in Elasticsearch.
...
In a production environment you would put APM Server on it's own machines,
...
Then configure the elastic-apm-node module by adding the following lines to the very top of your code:
