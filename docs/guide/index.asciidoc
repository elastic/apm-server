...
Regardless of whether your code is best described as a service or as an application, this guide will use indistinctly
the word service.

With APM, you can automatically collect detailed performance information from inside your service with only minor
changes to your code.
For example, APM agents can measure the response time for incoming requests, time spent in database queries, calls
...
Additionally, agents provide an API that you can use to manually instrument anything else you find interesting.
...
To visualize the data after it's sent to Elasticsearch, you can use the pre-built open source Kibana dashboards
that come with APM Server; or the dedicated APM UI bundled in X-Pack.

We designed Elastic APM to run on anything from a regular laptop to thousands of machines,
and it's easy to get started with.
...
* For real-user-monitoring (beta), data is being collected in agents running in browsers.
...
NOTE: If you are using an X-Pack secured version of Elastic Stack (enabled by default),
...
You can configure APM Server to listen on a UNIX domain socket.
[[kibana-dashboards]]
[float]
