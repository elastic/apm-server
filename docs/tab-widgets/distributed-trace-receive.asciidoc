...
[source,php]
----
$distData = ElasticApm::getCurrentTransaction()->getDistributedTracingData();
----

2. Serialize the current distributed tracing context to format supported by the request's transport and send it to the server side (i.e., the side receiving the request)

3. Receive the distributed tracing context on the server side and unserialize it back to a new `DistributedTracingData` object

4. Begin a new transaction using public API (for example {apm-php-ref}/public-api.html#api-elasticapm-class-begin-current-transaction[`ElasticApm::beginCurrentTransaction`])
and passing the new `DistributedTracingData` object as a parameter. For example:
...
5. Don't forget to eventually end the transaction on the server side
