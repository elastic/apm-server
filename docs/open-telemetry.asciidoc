...
The example below sets the Elastic (ECS) `service.environment` field for the resource, i.e. service, that is producing trace events:
...
The example below sets the `label.foo` field for events:
