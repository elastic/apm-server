...
Resource attributes are mapped to Elastic Common Schema (ECS) fields that describe the service and the environment that the service runs in.

This example sets the Elastic (ECS) `service.environment` field for the resource, i.e. service, that is producing trace events:
...
Use the {ot-attr}[attributes processor] to add data to span, log, or metric events.
Attributes set via this method are mapped to the ECS `label` field.

This example sets the `label.foo` field for events to `bar`:
