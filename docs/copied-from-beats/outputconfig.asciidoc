...
>>>>>>> 1a4f76ba... [docs] Update index prefix in output config (#2024)
...
When loading the pre-built Kibana dashboards via {beatname_uc},
...
NOTE: When changing APM indices, you have to update the {kibana-ref}/apm-settings-kb.html[Kibana APM settings]
accordingly to ensure the Kibana UI can still query the data.
