...
For Elastic Cloud Service deployments and ECE, the secret token is already provisioned on creation of deployment. The secret token can be found and reset in the deployments APM page.
