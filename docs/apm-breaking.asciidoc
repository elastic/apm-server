...
[float]
==== `ecs.version` is now a `constant_keyword` field
Instead of adding `ecs.version` as a field in every document written by APM Server,
it is now a `constant_keyword` field with its value set in data stream mappings.

This change removes `ecs.version` from `_source`. It is now only available through
the {ref}/script-fields-api.html[{es} `field` API].
APM does not currently rely on the `ecs.version` field, so there should be no noticeable change.

For more details, see https://github.com/elastic/apm-server/pull/9208[PR #9208].
...
[float]
==== `apm-server apikey` commands deprecated
The `apm-server apikey` commands have been deprecated.
API keys should be managed through either the Kibana or Elasticsearch REST APIs;
see <<api-key>> to learn how.

For more details, see https://github.com/elastic/apm-server/pull/9446[PR #9446].
// end::86-bc[]
