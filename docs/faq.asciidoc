...
[partintro]
--
This section contains frequently asked questions about APM Server. 
...
Not what you were looking for? Check out our <<getting-help>> section. 
--

[[why-503]]
== Why am I seeing an "HTTP 503"?

The APM Server has configurable limits on how many requests can be processed and 
how many documents can be ingested to Elasticsearch concurrently.
If the APM Server hits the limits, it returns an HTTP status code 503 and a message.
...
[float]
=== Possible Solutions
There are several options how you can fix underlying issues causing an APM Server to respond with 503:
...
If this happens, you can remove old indices to make room for new data.
To do this you can use a tool like Curator and set up a cron job to run it periodically.
For more details and an example have a look at <<delete-old-data-periodically>>.
