...
A non-sampled trace drops all <<transaction-spans,span>> and <<transactions,transaction>> data^1^.
Regardless of the sampling decision, all traces retain <<errors,error>> data.
