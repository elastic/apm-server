...
* *Sourcemap* (`sourcemap:write`): Required for uploading sourcemaps to {kib}.
...
In addition, if you're creating an API key from the APM UI,
...
POST /_security/role/apm_agent_key_role
{
   ...
   "applications": [
      ...
      { <1>
         ...
         "resources":[ "space:default" ] <2>
      }
   ]
}
----
<1> The privileges added in this object only apply to roles that will be creating API keys in the APM UI.
If you don't have access to the APM UI, you can omit this object and skip to <<create-an-api-key-alternate>>.
<2> Assigns privileges for the default space.
...
Or, to create an API key that can be used to upload RUM sourcemaps to {kib}, select `sourcemap:write`
(and then be sure to read our <<source-map-how-to,how to upload source maps>> guide).
...
[[create-an-api-key-alternate]]
[float]
=== Create an API key in {kib} Stack Management

If you don't have access to the APM UI, API keys can be created in {kib} Stack Management.
