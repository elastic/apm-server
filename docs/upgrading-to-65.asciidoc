...
Check the <<agent-server-compatibility>> for more details on which APM agents are compatible with APM Server version 6.5.
...
These new keys are essential to taking advantage of APM's new <<apm-distributed-tracing>> feature.
