...
If sensitive data has already been ingested into the {stack}, it's possible to remove or redact it.
...
to the default APM pipeline. Each APM data stream has its own default pipeline depending on the type of data being ingested:
...
In this case, we're interested in redacting application tracing data,
...
WARNING: Do not edit any of the predefined pipeline steps or you may break the APM integration and APM UI.
...
You will need to readd custom pipelines after every upgrade.
...
If sensitive data has already been ingested into the {stack}, it's possible to remove or redact it.
The following fields should be reviewed regularly to ensure sensitive data is not being captured:
...
+
Use the *remedy* column of the sensitive fields table above to determine how to stop collecting
the sensitive data.

. Delete or redact the ingested data.
+
With data collection fixed, you can now delete or redact the offending data:
+
* <<redact-field-data>>
* <<delete-doc-data>>
