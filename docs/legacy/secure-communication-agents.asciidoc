...
If you've already upgraded, see <<secure-agent-communication>>.
...
IMPORTANT: {deprecation-notice-config}
If you've already upgraded, see <<agent-tls>>.
...
* *Source map* (`sourcemap:write`): Required for <<sourcemaps,uploading source maps>>.
...
IMPORTANT: {deprecation-notice-config}
If you've already upgraded, see <<secret-token>>.
